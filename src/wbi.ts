...
export function mixinKey(orig: string): string {
  if (orig.length < MIXIN_KEY_SOURCE_LENGTH) throw new Error('wbi: key material must be at least 64 characters')
  let result = ''
  for (const index of MIXIN_KEY_ENC_TAB) result += String(orig[index])
...
export function wbiSign(
  ...
  const withWts: Record<string, string | number> = { ...params, wts: nowSec }
  ...
  const signed: Record<string, string> = { w_rid: wRid }
  for (const [key, value] of Object.entries(withWts)) signed[key] = String(value)
  return signed
}
