...
const SEARCH_ORDERS: readonly BilibiliSearchOrder[] = ['totalrank', 'click', 'pubdate', 'dm']
...
export function parseSearchArgs(
  args: { keyword: string; page?: number; pageSize?: number; order?: string },
  maxPageSize: number,
): BilibiliSearchArgs {
  if (args.keyword.trim().length === 0) throw new Error('keyword must be a non-empty string')
  ...
  if (pageSize > maxPageSize) throw new Error('pageSize exceeds the maximum of ' + String(maxPageSize))
  const order = args.order === undefined
    ? undefined
    : SEARCH_ORDERS.includes(args.order as BilibiliSearchOrder) ? args.order as BilibiliSearchOrder : undefined
  if (args.order !== undefined && order === undefined) throw new Error('order must be one of ' + SEARCH_ORDERS.join(', '))
 ...
 */
export function formatSearchOutput(result: BilibiliSearchResult): string {
  if (result.items.length === 0) {
    return 'No videos found for this query on bilibili.'
  }
  const lines = result.items.map((item: BilibiliVideoSummary) => {
    const meta: string[] = []
    if (item.author.length > 0) meta.push('UP: ' + item.author)
    meta.push('play: ' + String(item.play))
    if (item.duration.length > 0) meta.push('duration: ' + item.duration)
    if (item.publishAt.length > 0) meta.push('published: ' + item.publishAt.slice(0, 10))
 ...
 */
export function presentSearchCall(args: { keyword: string }): GenericCallView {
 ...
 */
export function presentSearchResult(_args: unknown, _result: ToolResult): undefined {
  return undefined
}
 ...
 */
export function applyBilibiliSearchTool(ctx: Context, service: BilibiliProviderLike, maxPageSize: number, timeoutMs: number): void {
 ...
 */
export function parseVideoArgs(args: { bvid: string }): { bvid: string } {
  const bvid = args.bvid.trim()
  if (bvid.length === 0) throw new Error('bvid must be a non-empty string')
  if (!BVID_PATTERN.test(bvid)) throw new Error('bvid must look like BV followed by 10 letters or digits (e.g. BV1GJ411x7h7)')
  return { bvid }
}
 ...
 */
export function formatVideoOutput(detail: BilibiliVideoDetail): string {
  const lines = [
    'Title: ' + detail.title,
  ...
  if (detail.typeName.length > 0) lines.push('Partition: ' + detail.typeName)
  if (detail.pages.length > 0) {
    const pageText = (page: typeof detail.pages[number]): string =>
      '#' + String(page.page) + ' ' + page.part + ' (cid ' + String(page.cid) + ', ' + String(page.duration) + 's)'
    lines.push('Parts: ' + detail.pages.map(pageText).join('; '))
  }
  if (detail.description.length > 0) lines.push('Description:\n' + detail.description)
  return lines.join('\n')
}
 ...
 * @returns the generic card view shown while the call runs.
 */
export function presentVideoCall(args: { bvid: string }): GenericCallView {
 ...
 * @returns always undefined, selecting the generic fallback.
 */
export function presentVideoResult(_args: unknown, _result: ToolResult): undefined {
  return undefined
}
 ...
 * @param timeoutMs - the cooperative tool-call budget (ms).
 */
export function applyBilibiliVideoTool(ctx: Context, service: BilibiliProviderLike, timeoutMs: number): void {
  ctx.tools.register(defineTool({
    name: 'bilibili_video',
 ...
 */
export function parseSubtitleArgs(args: { bvid: string; language?: string }): { bvid: string; language?: string } {
  const bvid = args.bvid.trim()
  if (bvid.length === 0) throw new Error('bvid must be a non-empty string')
  if (!BVID_PATTERN.test(bvid)) throw new Error('bvid must look like BV followed by 10 letters or digits (e.g. BV1GJ411x7h7)')
  const language = args.language === undefined ? undefined : args.language.trim()
  if (language !== undefined && language.length === 0) throw new Error('language must be a non-empty string')
 ...
 */
export function boundTranscript(transcript: string, maxChars: number): { transcript: string; truncated: boolean } {
  if (transcript.length <= maxChars) return { transcript, truncated: false }
 ...
 */
export function subtitleOutput(result: BilibiliSubtitlesResult, maxChars: number): BilibiliSubtitleOutput {
 ...
 */
export function formatSubtitleOutput(output: BilibiliSubtitleOutput): string {
 ...
 * @returns the generic card view shown while the call runs.
 */
export function presentSubtitleCall(args: { bvid: string }): GenericCallView {
  return { card: 'generic', title: args.bvid, kind: 'search', rawInput: args.bvid }
}
 ...
 * @returns always undefined, selecting the generic fallback.
 */
export function presentSubtitleResult(_args: unknown, _result: ToolResult): undefined {
  return undefined
}
 ...
 * @param timeoutMs - the cooperative tool-call budget (ms).
 */
export function applyBilibiliSubtitleTool(ctx: Context, service: BilibiliProviderLike, maxChars: number, timeoutMs: number): void {
  ctx.tools.register(defineTool({
